...
class EscposAndroidModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
          ...
          synchronized(this) {
            ...
            if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
              if (usbManager != null && usbDevice != null && mWritePromise != null) {
                ...
                var printer = EscPosPrinter(context, usbConnection, 203, 48f, 32)
                if (mWriteRaw != null) {
  ...
  fun requestPermission(promise: Promise) {
    ...
    var usbConnection = UsbPrintersConnectionsManager.selectFirstConnected(context)
  ...
  fun write(params: ReadableMap, promise: Promise) {
    ...
    if (usbConnection != null && usbManager != null) {
      ...
      promise.reject("UNSPECIFIED", "usb does not available")
    }
  }
}
